...
export PYTHONPATH=${PYTHONPATH:-} # let eval_libero find the LIBERO tools
export PYTHONPATH=$(pwd):${PYTHONPATH:+${PYTHONPATH}:}${LIBERO_HOME} # let LIBERO find the websocket tools from starVLA repo
###########################################################################################
...
video_out_path="${model_root}/videos_instr/${task_suite_name}"
log_path="${model_root}/logs_instr/${task_suite_name}"
mkdir -p "$video_out_path"
mkdir -p "$log_path"



${LIBERO_python} ./examples/LIBERO/eval_files/eval_libero_instr.py \
